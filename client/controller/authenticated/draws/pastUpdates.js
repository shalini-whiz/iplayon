Template.pastUpdates.onCreated(function() {
...
Template.pastUpdatesEventName2Template.onCreated(function() {
	//this.subscribe("projects");

	this.subscribe("onlyLoggedIn");
	this.subscribe("liveUpdates")
	this.subscribe("pastEventsIDCategories",Session.get("pastupTourId"));
	this.prizeEventId = new ReactiveVar(0);
	//not used
	this.venueLatitude = new ReactiveVar(0);
	this.venueLongitude = new ReactiveVar(0);
...
Template.pastUpdates.onDestroyed(function() {
	Session.set("pastEventID", null);
	Session.set("pastEventID", undefined);
	Session.set("pastupTourId",null);
	Session.set("pastupTourId",undefined);
});

Template.pastUpdatesEventName2Template.helpers({
	lEvents: function() {
		//var liveUpdat=liveUpdates.find({}).fetch();
		if(Session.get("pastupTourId")!=undefined||Session.get("pastupTourId")!=null){
			var lEvents = pastEvents.find({"tournamentId" : Session.get("pastupTourId")}).fetch();
			if (lEvents.length!==0) {
				return lEvents;
			}
...
Template.pastUpdates.helpers({

	lEvents2: function() {
		var lEvents = pastEvents.find({"tournamentEvent" : true}).fetch();
		//var liveUpdat=liveUpdates.find({}).fetch();
		if (lEvents.length!==0) {
			return lEvents;
		}
	...
	liveUpdates :function(){
		...
		var liveUpdat=liveUpdates.findOne({"eventId":pastEventID},{$sort:{"liveUpdateMessageTime.liveUpdateTime":-1}});
...
Template.pastUpdates.onRendered(function() {
...
Template.pastUpdates.events({
	"change #liveUpEventName2": function(e) {
		...
		Session.set("pastupTourId",thisIds)
		//var instance = UI.renderWithData(Template.liveUpEventName2Template, { });
       // UI.insert(instance, $("#liveUpEventName")[0]);
      	Blaze.render(Template.pastUpdatesEventName2Template, $("#liveUpEventName")[0]);
        Session.set("pastEventID",undefined);
        Session.set("pastEventID",null)
        $(".sendLive").hide();
		$(".viewLiveUpdate").hide();
	...
	"change #liveUpEventName": function(e) {
		...
		Session.set("pastEventID",thisId)

		var lEvents = pastEvents.findOne({"eventSubId":thisId});
	...
	"click #sendLiveButton":function(e){
		...
		var thisId = $("#liveUpEventName").val()
		var userId = Meteor.users.findOne({"_id":Meteor.userId()})
		var lEvents = pastEvents.findOne({"eventSubId":thisId})//{$and: [{"eventOrganizer":userId.userId.toString()},{"eventSubId":thisId}]});
		/*if(kl==undefined){
			//alert("now eventsViewer")
			$(".sendLive").hide();
			$(".viewLiveUpdate").show();
				    ...
				    //$.get("http://api.timezonedb.com/?lat="+lEvents.timezoneIdEventLat+"&lng="+lEvents.timezoneIdEventLng+"&format=json&key=MR22FNA90DU3", function(data1, status){
            			...
            			// no if(moment(uf3*1000).startOf('day')<=moment(uf1*1000).startOf('day')&&moment(uf5*1000).startOf('day')>=moment(uf1*1000).startOf('day')){
							...
							Meteor.call("insertLiveUpdates",xData,function(e,response){
								if(response){
									$("#liveDescriptionText").val("");
									Session.set("pastEventID",xData.eventId)
    ...
    "click #liveUpdatesScoresOpen":function(e){
		...
		var lEvents = pastEvents.findOne({"_id":thisId});
		var drawConfiguration = $("#drawConfiguration").val();

		if(lEvents){
			var eventType = "past";
			if($("#liveUpEventName").val()!==null){
				Session.setPersistent("past_selectedSportFromLive",$("#liveUpEventName :selected").html());
			}
			else{
				Session.setPersistent("past_selectedSportFromLive",0);
			}
