...
Template.myEventsPast.onCreated(function() {
  ...
  self.autorun(function() {
    handle2 = Meteor.subscribeWithPagination("mpastEventsLimit", 8);
  });
  /*
 ...
 */
Template.myEventsPast.onDestroyed(function() {
  if(handle2)
    handle2.stop();
  handle2=false;
  Session.set("sortCriteriaPAst", null);
  Session.set("sortCriteriaPAst", undefined);
  Session.set("downlodPopArrayHelper_past",undefined);
...
/**
 * template helpers which connects myEventsPast.html.
 * @methodName : lEvent is a function to find events,
 */
Template.myEventsPast.helpers({
  /**
   ...
   */
  lEvent: function() {
    var sortCriteria = Session.get("sortCriteriaPAst");
    var lEventListing = "";
    var userId=Meteor.users.findOne({"_id":Meteor.userId()});
    if(userId!=undefined){
    Session.set("lEventLists", pastEvents.find({}).fetch());
    lEventListing = myPastEvents.find({"eventOrganizer":userId.userId.toString()}).fetch();
    if (sortCriteria) {
      if (sortCriteria == 1) {
        lEventListing = "";
        return myPastEvents.find({"eventOrganizer":userId.userId.toString()}, {
          sort: {
            "eventStartDate1":1
          }
        }).fetch();
...
/**
 * On rendered of template myEventsPast.html initialize css bootstrap slimScroll,
 * niceScroll
 */
Template.myEventsPast.onRendered(function() {
...
Template.myEventsPast.events({
  ...
  'click #editEvent': function(e) {
    ...
    Session.set('sortCriteria', null);
    Session.set('sortCriteria', undefined);
    var eventLists = myPastEvents.findOne({
  ...
  'click #deleteEvent': function(e, template) {
    ...
    var gEventLists = myPastEvents.find({
   ...
   */
  'click  #sortByEventDate_PAST': function(e) {
    e.preventDefault();

    Session.set("sortCriteriaPAst", 1);
  },
   ...
   */
  'click  #sortByProjectName_PAST': function(e) {
    e.preventDefault();
    Session.set("sortCriteriaPAst", 2);
  },
   ...
   */
  'click #sortByPlaceName_PAST': function(e) {
    e.preventDefault();
    Session.set("sortCriteriaPAst", 3);
  },
  ...
  'click #viewEventT': function(event) {
    ...
    var eventLists = myPastEvents.find({
  ...
  'scroll #eventDataParent1': function(e) {
   ...
   "click #downloadSubList":function(e){
        ...
        try{
          Session.set("downlodPopArrayHelper_past",undefined);
          Session.set("consolidateID_past",undefined);
          Session.set("consolidateName",undefined);
          var userId = Meteor.users.findOne({"_id":Meteor.userId()})
          var doa = myPastEvents.find({"eventOrganizer":userId.userId.toString()}).fetch();
          if(doa.length!=0){
        var  arrNew=[];
        var downlodPopArray=[];
            var filn = myPastEvents.findOne({"tournamentId":this.tournamentId});
            if(filn!=undefined&&filn.eventsUnderTournament!=undefined){
              //set the session as events of tournament
          Session.set("downlodPopArrayHelper_past",filn.eventsUnderTournament);
          Session.set("consolidateID_past",this.tournamentId);
          Session.set("consolidateName",filn.eventName);
          //render the popup
          if(Session.get("downlodPopArrayHelper_past")){
            Blaze.render(Template.downloadSubscribersCSVPast,$("#downloadPopRender")[0]);
                $('#downloadSubscribersCSVPast').modal({
              backdrop: 'static'
            });
          }
        }
...
Template.feeListHeader.helpers({
  ...
  "tournamentName":function(){
    ...
    var tournInfo = myUpcomingEvents.findOne({"tournamentId":tournId});
    if(tournInfo)
