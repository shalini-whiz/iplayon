 ...
 *
 */
Template.viewPastEvents.onCreated(function() {
	//this.subscribe("projects");
	this.subscribe("eventUploads");
	this.subscribe("onlyLoggedIn");
	this.subscribe("onlyLoggedInALLRoles")
	this.subscribe("pastEventsID",Router.current().params._PostId)
	this.subscribe("pasteventsLISTTorunament",Router.current().params._PostId)
});
 ...
 *
 */
Template.viewPastEvents.onRendered(function() {
	if (Meteor.user()) {
...
Template.viewPastEvents.helpers({
	...
	lEventDetails: function(e) {
		try{
		var lId = Router.current().params._PostId;
		var lEventDetails = pastEvents.findOne({
			"_id": lId
		});
 ...
 */
Template.viewPastEvents.events({
	/*
	...
	'click #eventSubscribe': function(e) {
		...
		//fetch event
		var eveType = pastEvents.findOne({
			"_id": gEventId
		});
		...
		var gEventLists = pastEvents.find({
			"_id": gEventId
		}).fetch();
	...
	'click #eventTournamentUpdates': function(e) {
		e.preventDefault();
		var details = pastEvents.findOne({"_id":Router.current().params._PostId});
	...
	'click #yesButtonSubTeamEvent': function(e) {
		...
		if(sAccept==true){
			...
			var gEventLists = pastEvents.findOne({
				"_id": gEventId
			});
 ...
 *
 */
Template.sponsorDownloadPast.onCreated(function(){
	this.subscribe("eventUploads");
 ...
 */
Template.sponsorDownloadPast.helpers({
	lDownloadSponsorPdf: function(){
		try{
			var lData, lData1;
			var lEvents = pastEvents.findOne({
				"_id": Router.current().params._PostId
			});
 ...
 */
Template.sponsorDownloadPast.events({
	/*
	...
	'click #sponsorDownloadOk':function(e){
		e.preventDefault()
		$("#sponsorDownloadPast").modal('hide');
	}
});