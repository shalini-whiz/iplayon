...
Template.tournSchedulerPastEvents.onCreated(function() {
...
Template.tournSchedulerPastEvents.onRendered(function() {
...
Template.tournSchedulerPastEvents.onDestroyed(function() {

})

Template.tournSchedulerPastEvents.helpers({
    notAdmin: function() {
        try {
...
Template.tournSchedulerPastEvents.events({
    ...
    "click #generateScheduleForTour":function(e){
        try{
            var check = checkBasictimeValidPast()
            if(check){
                var data = {
...
checkBasicDurationValidPast = function(){
...
checkBasictimeValidPast = function(){
    try{
                        ...
                        if( break2St.toString().trim().length != 0){
                            if( break2End.toString().trim().length != 0){
                                if(noOfTablesPerDay && noOfTablesPerDay.trim().length != 0){
                                    var check = checkBasicDurationValidPast()
                                    if(check){
                                        return true
                                    }else{
                                        return false
