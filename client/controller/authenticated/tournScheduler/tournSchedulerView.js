...
Template.tournSchedulerView.onCreated(function() {
...
Template.tournSchedulerView.onRendered(function() {
...
Template.tournSchedulerView.onDestroyed(function() {

})

Template.tournSchedulerView.helpers({
    notAdmin: function() {
        try {
    ...
    eventList:function(){
    	try{
    		if(Session.get("selectedTournId")){
    			var r = events.find({tournamentEvent:false}).fetch()
    			return r
    		}
    	}catch(e){
    		alert(e)
...
Template.tournSchedulerView.events({
    ...
    "click #downloadSched":function(){
        try{
            ...
            //"scheduleDataForAPTTA": function(tournamentId, eventName, selectedDate, type) {

            /*Meteor.call("scheduleDataForAPTTA",Session.get("selectedTournId"),Session.get("showtournsOnly"),Session.get("selectedDate"),Session.get("showtournsOnly"),function(e,res){
                if (res) {
...
Template.registerHelper("findDetailsUsersSchedule", function(data) {
    try{
        if(data != undefined && data != null){
           var s = ReactiveMethod.call("findDetailsUsersTeams",data)
           if(s && s.userName){
            return s.userName
           }
...
Template.registerHelper("indexSchedule",function(data){
    try{
        return parseInt(data + from + 1)    
    }catch(e){

    }
})

Template.registerHelper("scoresOfMatch", function(scoresA, scoresB, data3, playerA, playerB, winnerId) {
    try {
        var scores = ""
        if (data3.toLowerCase() == "completed") {
            if (scoresA && scoresB) {
                if (playerA == winnerId) {
                    var scoreInfo = " ";
