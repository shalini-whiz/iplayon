...
Meteor.methods({
    "validationForTeamFormatAndPlayersLengthIP": function(teamFormatId, playersDetails) {
        try {
...
Meteor.methods({
    "validationForEachCriteria_SchoolIP": function(teamFormatId, playersDetails,schoolId) {
        try {
            var formatId = teamFormatId;
            var messageValidations = [];
            if (teamsFormat.findOne({
            ...
            if (playersDetails.length == 0) {
                ...
                for (var i = 0; i < playersDetails.length; i++) {
                    if(playersDetails[i]&&playersDetails[i].teamEvent!=undefined&&playersDetails[i].teamEvent==true){
                        ...
                        var playerDetFromDB = userDetailsTT.findOne({
...
Meteor.methods({
    "validationForMandatoryPlayers_SchoolIP": function(teamFormatId, playersDetails) {
        try {
            var formatId = teamFormatId;
            mandPlayersArrPosition = [];
...
Meteor.methods({
    playerRegesterationTeam_SchoolIP: function(teamFormatId, playersDetails) {
        try {
            var teamMembers = [];
            var teamMemIDs = [];
            var teamDet = [];
...
//create team
Meteor.methods({
    "createTeamForGivenTeamFormatIdIP": async function(xdata, managerID, schoolID, sourceData, subscriptionForSchool) {
        try {
            var resp = "0"
            ...
            try{
                var res = await Meteor.call("saveNewTeamData_ForSchoolIP", xdata, managerID, schoolID, sourceData, subscriptionForSchool);
                if (res != "0") {
                    resp = res
                }
            }catch(e){
...
Meteor.methods({
    "subscribeToIndvidiualEvent_SchoolIP": function(xdata, individualEventId, teamEventId, tournamentId, schoolID) {
        try {
            var eventName;
                ...
                if (eventFeeSettingsFetch && eventFeeSettingsFetch.events && eventFeeSettingsFetch.singleEvents && eventFeeSettingsFetch.singleEventFees) {
                    ...
                    if (indexABBName >= 0) {
                        ...
                        for (var i = 0; i < xdata.length; i++) {
                            var userDet = userDetailsTT.findOne({
                                "userId": xdata[i]
                            });
                            var academyId = "other";
...
Meteor.methods({
    "suscribeToTeamEvent_SchoolIP": function(managerID, individualEventId, teamEventId, tournamentId, teamID, schoolID) {
        try {
            var eventName;
            var subscribedEvents = [];
                    ...
                    if (managerID) {
                        ...
                        if (userDet) {
                            if (teamEntriesFind == undefined) {
                                var r = playerTeamEntries.insert({
                                    playerId: managerID,
                                    academyId: academyId,
                                    parentAssociationId: parentAssociationId,
                                    associationId: associationId,
...
Meteor.methods({
    "createTeams_SchoolIP": async function(teamFormatId, playersDetails, schoolID, source, individualEventId, teamEventId, tournamentId) {
        try {
            var messageValidations = [];
                ...
                if (moment(moment(findtournament.eventSubscriptionLastDate1).format("YYYY-MM-DD")) >= moment(moment.tz(findtournament.timeZoneName).format("YYYY-MM-DD"))) {
                    ...
                    if (schoolID != undefined && schoolID != null && schoolID.trim().length != 0) {
                        ...
                        if (fetchSchoolDet && fetchSchoolDet.abbrevation) {
                            ...
                            try{
                                var res1 = await Meteor.call("validationForEachCriteria_SchoolIP", teamFormatId, playersDetails, schoolID);
                                if (res1) 
                                {
                                    ...
                                    if (res1 != "0") {
                                        ...
                                        schoolDetailsData = fetchSchoolDet.abbrevation + "-" + teamNameFormat
                                            ...
                                            try{
                                            var res2 =  await Meteor.call("validationForMandatoryPlayers_SchoolIP", teamFormatId, playersDetails);
                                            if (res2 != "0") {
                                                var message = {
                                                    message: res2
                                                }
                                                ...
                                                try{
                                                var res3 = await Meteor.call("playerRegesterationTeam_SchoolIP", teamFormatId, playersDetails);
                                                    if (res3 != '0' && schoolDetailsData != null && schoolDetailsData != undefined && schoolDetailsData.trim().length != 0) {
                                                        ...
                                                        if (res3 && res3[0] && res3[0].teamMembers && res3[0].teamMembers[0] && res3[0].teamMembers[0].playerId && res3[0].teamManager) {
                                                            ...
                                                            try{
                                                                var res4 = await Meteor.call("createTeamForGivenTeamFormatIdIP", Teamdata, managerID, schoolID, sourceData, subscriptionForSchool);
                                                                if (res4 == "0") {
                                                                    var message = {
                                                                    ...
                                                                    if (teamID) {
                                                                        try{
                                                                            var res5 = await Meteor.call("subscribeToIndvidiualEvent_SchoolIP", onlyMemberIDs, individualEventId, teamEventId, tournamentId, schoolID);
                                                                            if(res5) {
                                                                                try{
                                                                                    var res6 = await Meteor.call("suscribeToTeamEvent_SchoolIP", managerID, individualEventId, teamEventId, tournamentId, teamID, schoolID);
                                                                                    if(res6) {
...
Meteor.methods({
    "createTeamForSchoolWithArguementIP": async function(data,apiKey) {
        try {
                                                    ...
                                                    if(teamEveDet&&teamEveDet._id&&teamEveDet.projectId&&teamEveDet.projectId.length!=0){
                                                        ...
                                                        if (data && data.minNumberOfPlayers != undefined) {
                                                            if (data.minNumberOfPlayers==true) {
                                                                try{
                                                                    var res = await Meteor.call("createTeams_SchoolIP", teamFormatId, data.playersDetails, data.schoolId, data.source, individualEventId, teamEventId, tournamentId);
                                                                    if (res) {
                                                                        Meteor.call("deletePlayersFromCategoryIP",data.schoolId,data.eventName);
                                                                        response = res;
                                                                    }
                                                                }catch(e){
                                                                    eror = e.reason;
                                                                ...
                                                                try{
                                                                    var res = await Meteor.call("createTeams_SchoolTemporaryIP", teamFormatId, data.playersDetails, data.schoolId, data.source, individualEventId, teamEventId, tournamentId);
                                                                    if (res) {
                                                                        response = res;
                                                                        Meteor.call("deletePlayersFromCategoryIP",data.schoolId,data.eventName);
                                                                    } 
...
Meteor.methods({
    "deletePlayersFromCategoryIP":function(schoolId,eventName){
        try{
            var deleteCheck  = playerCategory.remove({schoolId:schoolId,category:eventName});
        }catch(e){
...
Meteor.methods({
    createTeams_SchoolTemporaryIP: async function(teamFormatId, playersDetails, schoolID, source, individualEventId, teamEventId, tournamentId) {
        try {
            var messageValidations = [];
            var teamsFormatID = teamFormatId;
            var teamIdReturn = "";
            ...
            if (findtournament && findtournamentSingle) {
                if (moment(moment(findtournament.eventSubscriptionLastDate1).format("YYYY-MM-DD")) >= moment(moment.tz(findtournament.timeZoneName).format("YYYY-MM-DD"))) {
                    ...
                    if (schoolID != undefined && schoolID != null && schoolID.trim().length != 0) {
                        ...
                        if (fetchSchoolDet && fetchSchoolDet.abbrevation) {
                            try{
                            var res = await Meteor.call("validationForTeamFormatAndPlayersLengthIP", teamFormatId, playersDetails);
                                if (res) 
                                {
                                    var schoolDetailsData = '';
                                    if (res != "0") {
                                        ...
                                        schoolDetailsData = fetchSchoolDet.abbrevation + "-" + teamNameFormat
                                        try{
                                            var res2 = await Meteor.call("playerRegesterationTeam_SchoolIP", teamFormatId, playersDetails);
                                            if (res2 != '0' && schoolDetailsData != null && schoolDetailsData != undefined && schoolDetailsData.trim().length != 0) {
                                                ...
                                                if (res2 && res2[0] && res2[0].teamMembers && res2[0].teamMembers[0] && res2[0].teamManager) {
                                                    ...
                                                    try{
                                                        var res3 = await Meteor.call("createTeamForGivenTeamFormatIdIP", Teamdata, managerID, schoolID, sourceData, subscriptionForSchool);
                                                        if (res3 == "0") {
                                                            var message = {
                                                                message: "Cannot create team"
                                                            }
                                                            ...
                                                            if (teamID) {
                                                                try{
                                                                    var res4 = await Meteor.call("subscribeToIndvidiualEvent_SchoolIP", onlyMemberIDs, individualEventId, teamEventId, tournamentId, schoolID);
                                                                    if(res4) {
                                                                        Meteor.call("sendEmailOfSchoolIP", tournamentId, schoolID, function(e, res) {});
                                                                    }
                                                                }catch(e){
...
Meteor.methods({
    "getTeamDetailsForSchoolIP": async function(data,apiKey) {
        try {
                            ...
                            if(data.schoolId){
                                ...
                                if(schoolDet&&schoolDet.state){
                                    ...
                                    if(toureveDet&&toureveDet._id){
                                                ...
                                                if(singleEveDet&&singleEveDet._id){
                                                    ...
                                                    if(teamEveDet&&teamEveDet._id&&teamEveDet.projectId&&teamEveDet.projectId.length!=0){
                                                        ...
                                                        if (data) {
                                                            ...
                                                            try{
                                                                var res = await Meteor.call("getTeamDetailsForSchool_teamsIP", data);
                                                                if (res) {
                                                                    response = res
                                                                }
                                                            }catch(e){
                                        ...
                                        var resultJson = {};
                                        resultJson["status"] = "failure";
                                        resultJson["response"] = "tournament is not created yet";
                                        return resultJson;
                                    }
                                } else{
                                    var resultJson = {};
...
//get team details
Meteor.methods({
    "getTeamDetailsForSchool_teamsIP": function(data) {
        try {
            var response;
            var eror;
            var messageValidations = [];
            var responseToSend = [];
            if (data.schoolId && data.teamsFormatID) {
                var schoolId = data.schoolId;
                var teamsFormatId = data.teamsFormatID;
                ...
                if (findSchoolDet) {
                    ...
                    if (teamFormatDet) {
                        ...
                        if (schoolTeam) {
                            ...
                            if (schoolTeam && schoolTeam.teamMembers && schoolTeam.teamMembers.length != 0) {
                                ...
                                for (var tm = 0; tm < teamMembersSchool.length; tm++) {
                                    ...
                                    if (mem && mem.playerId && mem.playerNumber) {
                                        var playerDet = userDetailsTT.findOne({
                                            "userId": mem.playerId
                                        });
                                        var playerName = "";
                            ...
                            if(residualPlayers&&residualPlayers.userId){
                                ...
                                for (var tm = 0; tm < residualPlayersSchool.length; tm++) {
                                    var mem = residualPlayersSchool[tm];
                                    if (mem) {
                                        var playerDet = userDetailsTT.findOne({
                                            "userId": mem
                                        });
...
Meteor.methods({
    "getTournamentDetailsForStateIP":function(data,apiKey){
        try {
            ...
            if(data&&apiKey){
                ...
                if(apiKeyDet&&apiKeyDet.userId){
                    ...
                    if(Meteor.users.findOne({userId:eveOrgID})){
                        if(data.stateId){
                            var domainDet = domains.findOne({"_id":data.stateId});
                            if(domainDet&&domainDet.domainName){
                                var toureveDet = events.findOne({"eventOrganizer":eveOrgID,"domainId":data.stateId,"tournamentEvent":true});
                                if(toureveDet&&toureveDet._id){
                                    var tournamentId = toureveDet._id;
                                    var eventName = "";
                                    var domainName = "";
